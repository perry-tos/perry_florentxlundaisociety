...
import { DEMO_NEW_TOS, DEMO_OLD_TOS } from '@/lib/mock-data';
import { useEffect, useRef, useState } from 'react';

type PipelineStage = 'idle' | 'crawling' | 'analyzing' | 'dispatching' | 'complete' | 'error';
...
const BRAIN_URL: string = process.env.NEXT_PUBLIC_BRAIN_URL ?? 'https://brain.perrylabs.se';
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-[28px] font-semibold tracking-tight">Live Demo</h1>
              <span className="text-[11px] font-mono text-white bg-foreground px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                Demo Mode
              </span>
            </div>
            <p className="text-[15px] text-muted">
              Simulate a ToS change and watch the full pipeline execute in real time
            </p>
          </div>
          ...
          <>
            ...
            <div className="bg-white border border-border-light rounded-2xl p-6 mb-6">
              <div className="flex items-center justify-between">
                {stages.map((s, i) => {
                  const stageIndex: number = stages.findIndex((x) => x.key === stage);
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white border border-border-light rounded-2xl overflow-hidden">
                  <div className="px-5 py-3 border-b border-border-light">
                    <span className="text-[13px] font-medium text-muted">AI Analysis</span>
                  </div>
                  {analysis ? (
                    <div className="p-5 space-y-3">
                      <div className="flex items-center gap-3">
                        <span className="text-[15px] font-semibold">{analysis.provider}</span>
                        <span
                      ...
                      <p className="text-[14px] text-muted leading-relaxed">{analysis.summary}</p>
                    </div>
                  ) : (
                ...
                {analysis?.packages.map((pkg) => (
                  ...
                  >
                    ...
                    <div className="p-5 space-y-4">
                      <p className="text-[14px] text-muted leading-relaxed">{pkg.summary}</p>
                      ...
                      {pkg.breaking_changes.length > 0 && (
                        <div className="space-y-2">
                          ...
                          {pkg.breaking_changes.map((change, i) => (
                            <div key={i} className="border-l-2 border-danger pl-3 space-y-1">
                        ...
                        <div className="space-y-2">
                          ...
                          {pkg.recommended_actions.map((action, i) => (
                            <div key={i} className="flex items-start gap-2 text-[13px]">
                              <span className="text-success mt-0.5 shrink-0 font-mono">→</span>
                ...
                <div className="bg-white border border-border-light rounded-2xl overflow-hidden">
                  <div className="px-5 py-3 border-b border-border-light flex items-center justify-between">
                    <span className="text-[13px] font-medium text-muted">Dispatch Status</span>
                    {broadcast && (
                      <span className="text-[12px] text-muted">
                  ...
                  {broadcast && broadcast.results.length > 0 ? (
                    <div className="divide-y divide-border-light">
                      {broadcast.results.map((row) => (
                        <div key={row.repo} className="px-5 py-3 flex items-center justify-between">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <span className="text-[13px] font-mono truncate">{row.repo}</span>
                          </div>
